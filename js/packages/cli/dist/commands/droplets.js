...
export function createDropletsCommand() {
    ...
    droplets
        ...
        try {
            ...
            console.log('%-8s %-20s %-10s %-12s %-10s %-15s %s', 'ID', 'NAME', 'STATUS', 'SIZE', 'REGION', 'IP', 'CREATED');
            ...
            for (const droplet of dropletList) {
                ...
                console.log('%-8d %-20s %-10s %-12s %-10s %-15s %s', droplet.id, droplet.name, droplet.status, droplet.size_slug, region, ip, created);
            }
        }
        catch (error) {
